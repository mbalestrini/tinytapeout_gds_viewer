...
let GDS = {
    ...
    addLayer: function (layer_number, layer_datatype, name, visual_order, color) {
        ...
        materialParams.color = new THREE.Color().setRGB(color[0], color[1], color[2], THREE.LinearSRGBColorSpace);
    ...
    addCell: function (cell_name, bounds, is_top_cell) {
        ...
        if (is_top_cell == 1)
            this.top_cells.push(cell_name);

    },

    addReference: function (parent_cell_name, cell_name, instance_name, ref_origin_x, ref_origin_y, ref_rotation, ref_x_reflection) {
        const matrix = new THREE.Matrix4();
        const translation = new THREE.Vector3(ref_origin_x, ref_origin_y, 0);
        const rotation = new THREE.Quaternion(0, 0, Math.sin(ref_rotation / 2.0), Math.cos(ref_rotation / 2.0));
