...
async function initialize() {
    ...
    self.onmessage = (event) => {
        if (event.data.type == WORKER_MSG_TYPE.PROCESS_GDS) {
            ...
            ModuleInstance.ccall('processGDS', null, ['string', 'number'], [event.data.filename, (event.data.opt_just_lines ? 1 : 0)]);
            ...
            ModuleInstance.ccall('addProcessLayer', null, ['number', 'number', 'string', 'number', 'number'], [event.data.layer_number, event.data.layer_datatype, event.data.name, event.data.zmin, event.data.zmax]);
        }
    };
    ...
    self.gds_add_lines = (cell_name, mesh_name, layer_number, layer_datatype, positions_count, positions_ptr, indices_count, indices_ptr) => {
        const positionsArray = new Float32Array(ModuleInstance.HEAPF32.buffer, positions_ptr, positions_count);
        const indicesArray = new Uint32Array(ModuleInstance.HEAP32.buffer, indices_ptr, indices_count);

        const lines_buffer = new ArrayBuffer(positions_count * Float32Array.BYTES_PER_ELEMENT + indices_count * Uint32Array.BYTES_PER_ELEMENT);
        const positionsView = new Float32Array(lines_buffer, 0, positions_count);
        const indicesView = new Uint32Array(lines_buffer, positions_count * Float32Array.BYTES_PER_ELEMENT, indices_count);
    ...
    self.gds_add_mesh = (cell_name, mesh_name, layer_number, layer_datatype, positions_count, positions_ptr, indices_count, indices_ptr) => {
        const positionsArray = new Float32Array(ModuleInstance.HEAPF32.buffer, positions_ptr, positions_count);
        const indicesArray = new Uint32Array(ModuleInstance.HEAP32.buffer, indices_ptr, indices_count);

        const mesh_buffer = new ArrayBuffer(positions_count * Float32Array.BYTES_PER_ELEMENT + indices_count * Uint32Array.BYTES_PER_ELEMENT);
        const positionsView = new Float32Array(mesh_buffer, 0, positions_count);
        const indicesView = new Uint32Array(mesh_buffer, positions_count * Float32Array.BYTES_PER_ELEMENT, indices_count);
    ...
    self.gds_add_label = (cell_name, layer_number, layer_datatype, text, origin_x, origin_y, pos_z) => {
        self.postMessage({
    ...
    self.gds_add_reference = (parent_cell_name, cell_name, instance_name, origin_x, origin_y, rotation, x_reflection) => {
        self.postMessage({
